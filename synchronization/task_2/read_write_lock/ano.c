...
int lock_init(node_t *node) { return pthread_rwlock_init(&(node->sync), NULL); }

int destroy_lock(node_t *node) { return pthread_rwlock_destroy(&(node->sync)); }

int rlock_node(node_t *node) { return pthread_rwlock_rdlock(&(node->sync)); }

int wlock_node(node_t *node) { return pthread_rwlock_wrlock(&(node->sync)); }

int unlock_node(node_t *node) { return pthread_rwlock_unlock(&(node->sync)); }
